...
cities['Chandigarh'] = ' Chandigarh | Mani Marja';
cities['Chhattisgarh'] =
...
const PaymentForm = props => {
    ...
    useEffect(() => {
        console.log('cites are:', cities);

        setStateCities(cities[`${state}`]);
        console.log('After update cites are:', cities);
    }, [state]);

    console.log('In payment', props);
    const onSubmitHandler = async e => {
        e.preventDefault();
        console.log(('in submithandlker:', formValues));
        const itemValues = props?.location?.state.values;
        console.log(itemValues);

        const response = await valley.post('order/', {
            items: { ...itemValues },
            formData: {
                ...
                mode_of_payment: 'cod',
                items: ['123', '123'],
                total_amount: props?.location?.state.cartTotal,
    ...
    return (
        <div className="payment__form">
            <div className="backdrop__form"></div>
            <h1 className="payment__form__heading">Enter Your Details to Checkout</h1>
